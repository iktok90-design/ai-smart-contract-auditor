#!/usr/bin/env node
/**
 * AuditSentry forge-runner MCP server.
...
const server = new Server({ name: "auditsentry-forge-runner", version: "0.1.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    if (name === "test") {
        ...
        const fa = ["test"];
        if (test)
            fa.push("--match-test", test);
        if (forkUrl)
            fa.push("--fork-url", forkUrl);
        if (flags)
            fa.push(...flags);
