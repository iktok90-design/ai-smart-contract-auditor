#!/usr/bin/env node
/**
 * AuditSentry tenderly MCP server.
...
async function tenderlyFetch(path: string, body?: unknown) {
  if (!KEY || !ACCT || !PROJ) {
...
const server = new Server(
  { name: "auditsentry-tenderly", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (name === "create_fork") {
    ...
    if (data.__mock) return textResult({ __stub: true, fork_id: "stub-fork-id", rpc: "https://rpc.tenderly.co/fork/STUB" });
    return textResult(data);
  }
  ...
  if (name === "delete_fork") {
    ...
    if (data.__mock) return textResult({ __stub: true, deleted: forkId });
    return textResult(data);
  }
