#!/usr/bin/env node
/**
 * AuditSentry gas-tracker MCP server.
...
async function fetchGasPrice(chain: string): Promise<{ chain: string; gwei: number | null; raw: string | null }> {
  const url = RPCS[chain];
  if (!url) return { chain, gwei: null, raw: null };
  try {
...
const server = new Server(
  { name: "auditsentry-gas-tracker", version: "0.1.0" },
  { capabilities: { tools: {} } }
);
